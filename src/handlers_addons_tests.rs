...
//
// Focuses on:
//   * The addon `catalog()` contents (contract with the frontend)
//   * The pure `build_resources_from_overrides` helper (fallback logic
//     between per-request overrides and per-addon defaults)
//   * `interpolate` / `resolve_default_env` — placeholder substitution
//     for `{deployment_name}` / `{namespace}` in default env values
//   * `inject_addon_env_into_primary` / `remove_injected_env_from_primary` —
//     the mutable helpers that keep the primary container's env vars in
//     sync with the sidecar's advertised connection info.
//
// # Handler-level testing
//
// The `attach` / `update` / `detach` handlers require a live kube client
// to exercise end-to-end. See `src/integration_tests.rs` for the
// HTTP-surface tests that cover those flows against a real cluster.
//
// To integrate, append to src/handlers/addons.rs:
//
//     #[cfg(test)]
//     #[path = "../../tests/handlers_addons_tests.rs"]
//     mod tests;
...
mod tests {
    ...
    fn build_resources_from_overrides_empty_spec_yields_none_map() {
        // An override that has all-None fields should still cause the fallback
        // (`or_else(|| defaults...)`) not to fire — it does fire because
        // `to_map` returns None for empty maps. Verify the collapsed result
        // is None when neither map has content.
        let r = build_resources_from_overrides(
            Some(ResourceSpec { cpu: None, memory: None }),
            Some(ResourceSpec { cpu: None, memory: None }),
            None,
        );
        assert!(r.is_none());
    ...
    //
    // The otel-collector addon relies on `{deployment_name}` substitution
    // in its default env values so the tracing backend automatically picks
    // up the target Deployment's name. Regressions here silently ship
    // literal `{deployment_name}` strings to production traces.
    ...
    //
    // Injects the addon's advertised env vars (e.g. REDIS_URL=…) into the
    // *first* container so the primary application can discover the
    // sidecar. Must be non-destructive: user-supplied env vars that share
    // a name must win, and the returned list must be exactly the names we
    // actually inserted so detach() can reverse the operation.
    ...
    fn inject_addon_env_never_overwrites_user_supplied_var() {
        ...
        let injected = inject_addon_env_into_primary(&mut pod, &addon);
        // Nothing was injected because the primary already had REDIS_URL,
        // so detach() has no work to do for this var either.
    ...
    fn remove_injected_env_clears_env_to_none_when_last_var_removed() {
        ...
        // env goes back to None (not `Some(vec![])`) so the resulting
        // PodSpec matches what a container with no env looks like.
        assert!(pod.containers[0].env.is_none());
    }
