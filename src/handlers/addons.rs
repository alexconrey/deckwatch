...
use crate::kube_ext::{deployment_detail, ingress_summary, pod_summary, IngressSummary, PodSummary};
...
#[derive(Clone, Serialize)]
pub struct EnvVarOutput { pub name: String, pub value: String }

#[derive(Clone, Serialize)]
pub struct ResourceSpecOutput { pub cpu: Option<String>, pub memory: Option<String> }

#[derive(Serialize)]
pub struct AddonListResponse { pub addons: Vec<AddonDefinition> }

#[derive(Deserialize, Default)]
pub struct AttachAddonRequest {
    #[serde(default)] pub container_name: Option<String>,
    #[serde(default)] pub port: Option<i32>,
    #[serde(default)] pub env: Option<Vec<EnvVarInput>>,
    #[serde(default)] pub resource_limits: Option<ResourceSpec>,
    #[serde(default)] pub resource_requests: Option<ResourceSpec>,
}
...
fn catalog() -> Vec<AddonDefinition> {
    vec![
        AddonDefinition {
            id: "redis".to_string(), name: "Redis".to_string(),
            description: "In-memory key-value store, useful as a cache or ephemeral state sidecar.".to_string(),
            image: "redis:7-alpine".to_string(), default_port: Some(6379),
            default_env: vec![EnvVarOutput {
                name: "REDIS_URL".to_string(),
                value: "redis://localhost:6379".to_string(),
            }],
            default_resources: Some(ResourceSpecOutput { cpu: Some("100m".to_string()), memory: Some("128Mi".to_string()) }),
        },
        AddonDefinition {
            id: "memcached".to_string(), name: "Memcached".to_string(),
            description: "Distributed memory caching sidecar.".to_string(),
            image: "memcached:1.6-alpine".to_string(), default_port: Some(11211),
            default_env: vec![EnvVarOutput {
        ...
        AddonDefinition {
            id: "nginx-proxy".to_string(), name: "Nginx Proxy".to_string(),
            description: "Nginx sidecar for TLS termination or caching in front of the primary container.".to_string(),
            image: "nginx:1.27-alpine".to_string(), default_port: Some(8080),
            default_env: vec![EnvVarOutput {
                name: "PROXY_URL".to_string(),
                value: "http://localhost:8080".to_string(),
            }],
            default_resources: Some(ResourceSpecOutput { cpu: Some("50m".to_string()), memory: Some("64Mi".to_string()) }),
        },
        AddonDefinition {
            id: "fluent-bit".to_string(), name: "Fluent Bit".to_string(),
            description: "Lightweight logging sidecar that forwards container logs downstream.".to_string(),
            image: "fluent/fluent-bit:3.1".to_string(), default_port: None, default_env: vec![],
            default_resources: Some(ResourceSpecOutput { cpu: Some("50m".to_string()), memory: Some("64Mi".to_string()) }),
        },
        AddonDefinition {
            id: "otel-collector".to_string(), name: "OpenTelemetry Collector".to_string(),
            description: "Sidecar that collects and forwards traces/metrics to the cluster tracing backend.".to_string(),
            image: "otel/opentelemetry-collector:latest".to_string(), default_port: Some(4317),
...
fn resolve_default_env(def_env: &[EnvVarOutput], ctx: &InterpolationCtx) -> Vec<EnvVarOutput> {
    def_env.iter().map(|v| EnvVarOutput {
        name: v.name.clone(),
...
pub async fn attach(
    ...
    let def = catalog().into_iter().find(|a| a.id == addon_id)
    ...
    let container_name = overrides.container_name.clone()
    ...
    if dep.metadata.annotations.as_ref()
        .map(|a| a.contains_key(&annotation_key))
        .unwrap_or(false)
    {
        return Err(AppError::BadRequest(format!("addon '{addon_id}' already attached as '{container_name}'")));
    }

    let template = dep.spec.as_mut().map(|s| &mut s.template)
        .ok_or_else(|| AppError::BadRequest("deployment has no pod template".to_string()))?;

    let template_annotations = template.metadata.get_or_insert_with(Default::default)
        .annotations.get_or_insert_with(BTreeMap::new);
    template_annotations.insert(annotation_key.clone(), def.id.clone());

    let pod_spec = template.spec.as_mut()
    ...
    if pod_spec.containers.iter().any(|c| c.name == container_name) {
        return Err(AppError::BadRequest(format!("container '{container_name}' already exists")));
    }

    let ctx = InterpolationCtx { deployment_name: &name, namespace: &ns };
    let resolved_default_env = resolve_default_env(&def.default_env, &ctx);

    let port = overrides.port.or(def.default_port);
    let ports = port.map(|p| vec![ContainerPort { container_port: p, ..Default::default() }]);
    let env: Vec<EnvVar> = overrides.env.map(|vars| {
        vars.into_iter().map(|v| EnvVar { name: v.name, value: Some(v.value), ..Default::default() }).collect()
    }).unwrap_or_else(|| {
        resolved_default_env.iter().map(|v| EnvVar { name: v.name.clone(), value: Some(v.value.clone()), ..Default::default() }).collect()
    });

    let resources = build_resources_from_overrides(overrides.resource_requests, overrides.resource_limits, def.default_resources.as_ref());
    ...
    if !injected_names.is_empty() {
        let template_annotations = template.metadata.as_mut()
...
pub async fn update(
    ...
    let annotations = template.metadata.as_mut().and_then(|m| m.annotations.as_mut())
        .ok_or_else(|| AppError::NotFound(format!("addon '{addon_id}' is not attached")))?;
    let container_name = annotations.iter()
    ...
    let previously_injected: Vec<String> = annotations.get(&injected_key)
        .map(|v| v.split(',').filter(|s| !s.is_empty()).map(|s| s.to_string()).collect())
        .unwrap_or_default();

    let pod_spec = template.spec.as_mut()
        .ok_or_else(|| AppError::BadRequest("deployment has no pod spec".to_string()))?;
    let sidecar = pod_spec.containers.iter_mut().find(|c| c.name == container_name)
        .ok_or_else(|| AppError::NotFound(format!("addon container '{container_name}' not found in pod spec")))?;

    if let Some(port) = overrides.port {
        sidecar.ports = Some(vec![ContainerPort { container_port: port, ..Default::default() }]);
    }

    let env_changed = overrides.env.is_some();
    let new_env_for_injection: Vec<EnvVarOutput> = if let Some(vars) = overrides.env {
        let out: Vec<EnvVarOutput> = vars.iter().map(|v| EnvVarOutput { name: v.name.clone(), value: v.value.clone() }).collect();
        let env: Vec<EnvVar> = vars.into_iter()
            .map(|v| EnvVar { name: v.name, value: Some(v.value), ..Default::default() })
            .collect();
...
pub async fn detach(
        ...
        Some(a) => {
            ...
            match key {
                Some(k) => {
                    ...
                    let injected_key = format!("{ADDON_INJECTED_ENV_ANNOTATION_PREFIX}{container_name}");
                    let injected: Vec<String> = a.remove(&injected_key)
                        .map(|v| v.split(',').filter(|s| !s.is_empty()).map(|s| s.to_string()).collect())
                        .unwrap_or_default();
                    (container_name, injected)
                }
                None => return Err(AppError::NotFound(format!("addon '{addon_id}' is not attached"))),
            }
        }
        None => return Err(AppError::NotFound(format!("addon '{addon_id}' is not attached"))),
    };
    ...
    if pod_spec.containers[0].name == container_name {
        return Err(AppError::BadRequest("addon container is the primary container; refusing to remove".to_string()));
    }
    ...
    if let Some(dep_annotations) = dep.metadata.annotations.as_mut() {
        ...
        dep_annotations.remove(&format!("{ADDON_INJECTED_ENV_ANNOTATION_PREFIX}{container_name}"));
    }
...
fn build_resources_from_overrides(
    ...
    let to_map = |spec: ResourceSpec| {
        ...
        if let Some(cpu) = spec.cpu { m.insert("cpu".to_string(), Quantity(cpu)); }
        if let Some(memory) = spec.memory { m.insert("memory".to_string(), Quantity(memory)); }
        if m.is_empty() { None } else { Some(m) }
    };
    let req_spec = requests.or_else(|| defaults.map(|d| ResourceSpec { cpu: d.cpu.clone(), memory: d.memory.clone() }));
    let lim_spec = limits.or_else(|| defaults.map(|d| ResourceSpec { cpu: d.cpu.clone(), memory: d.memory.clone() }));
    ...
    if requests_map.is_some() || limits_map.is_some() {
        Some(ResourceRequirements { requests: requests_map, limits: limits_map, ..Default::default() })
    } else { None }
}

async fn list_pods_for_deployment(state: &AppState, ns: &str, dep: &Deployment) -> Result<Vec<PodSummary>, AppError> {
    let pods_api = state.pods_api(ns)?;
    let selector = dep.spec.as_ref().and_then(|s| s.selector.match_labels.as_ref())
...
async fn list_ingresses_for_service(state: &AppState, ns: &str, service_name: &str) -> Result<Vec<IngressSummary>, AppError> {
    let ing_api = state.ingresses_api(ns)?;
    let t = K8sTimer::new("ingresses", "list");
    let all = ing_api.list(&ListParams::default()).await;
    t.finish(all.is_ok());
    let all = all?;
    Ok(all.iter().filter(|ing| {
        ing.spec.as_ref().and_then(|s| s.rules.as_ref()).map(|rules| {
            rules.iter().any(|r| r.http.as_ref().map(|http| {
                http.paths.iter().any(|p| p.backend.service.as_ref().map(|s| s.name == service_name).unwrap_or(false))
            }).unwrap_or(false))
        }).unwrap_or(false)
    }).map(ingress_summary).collect())
}
