...
fn deckwatch_tool_definitions() -> Vec<serde_json::Value> {
    vec![
        ...
        serde_json::json!({
            ...
            "description": "Generate a local docker run command that reproduces a deployment's Kaniko build locally. Useful for diagnosing build failures without pushing commits. Returns the exact docker command with all flags matching what deckwatch would use in-cluster.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "namespace": { "type": "string" },
                    ...
                    "local_context": { "type": "string", "description": "Local path to the repo/build context (default: current directory '.')" },
                    "no_push": { "type": "boolean", "description": "Add --no-push flag to skip pushing the image (default: true)" }
                },
                "required": ["namespace", "deployment_name"],
                "additionalProperties": false
            }
...
async fn tool_generate_local_build(
    ...
    let mut kaniko_args = vec![
        format!(
            ...
            dockerfile.split('/').last().unwrap_or(dockerfile)
        ),
    ...
    let result = serde_json::json!({
        ...
        "notes": [
            ...
            format!("To test arm64: change --platform to linux/arm64 (requires Docker buildx or arm64 host)"),
