...
//
// To integrate, append to src/config.rs:
//
//     #[cfg(test)]
...
mod tests {
    ...
    fn allowed_namespaces_filters_empty_entries() {
        ...
        let cfg =
            Config::try_parse_from(["deckwatch", "--namespaces", "team-a,,team-b,"]).unwrap();
    ...
    fn frontend_dir_override() {
        let cfg =
            Config::try_parse_from(["deckwatch", "--frontend-dir", "/opt/deckwatch/dist"])
                .unwrap();
