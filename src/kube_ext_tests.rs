...
use k8s_openapi::api::core::v1::{
    ...
    ContainerStateWaiting, ContainerStatus, EnvVar, ExecAction, HTTPGetAction, Node, NodeCondition,
    NodeStatus, NodeSystemInfo, Pod, PodCondition, PodSpec, PodStatus, PodTemplateSpec, Probe,
...
#[test]
fn deployment_phase_scaled_to_zero_when_desired_zero() {
...
fn deployment_detail_captures_conditions() {
    ...
    assert_eq!(
        detail.conditions[1].reason.as_deref(),
        Some("NewReplicaSetAvailable")
    );
}
...
fn pod_summary_not_ready_when_any_container_not_ready() {
    ...
    assert_eq!(
        sum.container_statuses[1].state_reason.as_deref(),
...
fn ingress_path_defaults_when_missing() {
    let mut ing = make_ingress();
    let path = &mut ing.spec.as_mut().unwrap().rules.as_mut().unwrap()[0]
        .http
        .as_mut()
        .unwrap()
        .paths[0];
    ...
    assert_eq!(d.rules[0].paths[0].service_port, 0);
...
fn node_summary_not_ready_status() {
    ...
    // Returns empty when labels is None (no labels map at all).
