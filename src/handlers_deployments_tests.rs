...
//
// These helpers are module-private, so this test module must live inside
// the handlers::deployments module. To integrate, append this to
// src/handlers/deployments.rs:
//
//     #[cfg(test)]
//     #[path = "../../tests/handlers_deployments_tests.rs"]
//     mod tests;
//
// (Or copy the `#[cfg(test)] mod tests { ... }` block below into
// src/handlers/deployments.rs verbatim.)
//
// # Handler-level testing
//
// The handler functions themselves (`list`, `create`, `update`, etc.)
// take `State<AppState>` which wraps a `kube::Client`. There is no
// production-ready way to build a `kube::Client` offline without either
// (a) spinning up a real cluster (see `src/integration_tests.rs` and
// `README_INTEGRATION.md`) or (b) writing a mock HTTP transport for the
// underlying `tower::Service`. This test file therefore focuses on the
// pure helpers; handler-flow coverage lives in the integration tests.
...
mod tests {
    ...
    // ---- resolve_ports ----
    //
    // The legacy single-`port` field must keep working (older SPA builds
    // still send it) while the new `ports` array wins whenever it's set.
    // Protocol strings must be upper-cased because the k8s API rejects
    // lowercase `tcp`/`udp`.
    ...
    fn resolve_ports_empty_array_yields_none() {
        // An explicit-but-empty ports array is treated the same as "none
        // specified" — otherwise we'd emit `containers[].ports: []` which
        // some k8s validators complain about.
    ...
    fn resolve_ports_empty_name_and_protocol_strings_are_dropped() {
        // The frontend sometimes sends `""` for name/protocol when the
        // user cleared the field. We filter those out so the k8s API
        // sees `None` rather than an invalid empty string.
